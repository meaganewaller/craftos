...
module Scraper
  module Colorways
    class Malabrigo < Base
      register "malabrigo"

      COLORWAY_URL = "https://malabrigoyarn.com/yarns"
      ...
      def scrape(client:, brand_slug:, yarn_slug:)
        ...
        doc.css(".color-swatch, .colorway, [data-color]").filter_map do |el|
          name = el["data-color"] || el["title"] || el.css(".color-name, .name").first&.text
        ...
        puts "  [Colorway] Malabrigo/#{yarn_slug} failed: #{e.message}"
        []
      end
      ...
      def classify_color(name)
        ...
        when /blue|navy|sky|cobalt|azul/ then "blue"
